function Calendar(container) {
  ...
  function generateMonthBody(){
    ...
    for (i = 0; i < monthFirstDay - 1; i++){ // monthFirstDay starts from 0 for Sunday
    ...
    for (i = 0; i < monthLength; i++){ // 7 days * 6 weeks = 42
      var isSelected = (new Date(self.selectedDay.getFullYear(), self.selectedDay.getMonth(), self.selectedDay.getDate())).getTime() == (new Date(self.year, self.month, i+1)).getTime();

      monthStr += "<div class='calendar__day" + (isSelected ? " calendar__day--selected" : "")+ "'><span>" + (i + 1) + "</span></div>";
    }
